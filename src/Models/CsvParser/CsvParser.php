...
namespace nordgen\DbBatch\Models\CsvParser;
...
use LimitIterator;
use nordgen\DbBatch\Models\CsvParser\Iterator\CsvIterator;
use nordgen\DbBatch\Models\CsvParser\Iterator\FileIterator;
use Traversable;
...
class CsvParser implements IteratorAggregate
{
    ...
    public function __construct(Iterator $csv, array $option = array())
    {
        ...
        // Use first line as header if wanted
        if (isset($option['header']) && $option['header'] == true) {
            $option['header'] = (array)$this->iterator->current();
            $this->iterator = new CsvIterator($csv, $option);
            //$this->iterator->next();
        }
