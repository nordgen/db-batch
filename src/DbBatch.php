            ...
            public function insertRowIntoTable($table, $row, $rownum, $rowPopulator, &$extraData = []) {
                switch ($this->getConnectionType()) {
                    case 'ADODB' :
                        ...
                        $rs = $this->getDb()->Execute ( $sql ); // Execute the query and get the empty recordset
                        ...
                        if (!!$rowToInsert) {
                            $insertSQL = $this->getDb()->GetInsertSQL ( $rs, $rowToInsert );
                            $result = $this->getDb()->Execute ( $insertSQL ); // Insert the record into the database;
                            if (!$result && $isThrowExceptionEnabled) {
                                throw new \Exception($this->getDb()->ErrorMsg());
                            }
                            return !!$result;
                        }
                    ...
                    case 'yii\\db\\Connection' :
                        ...
                        // Ignore row if it is false
                        if (!!$rowToInsert) {
                            $this->getDb()->createCommand ()->insert ( $table, $rowToInsert );
                            
                            if ($isThrowExceptionEnabled) {
                                return !!$this->getDb()->createCommand ()->insert ( $table, $rowToInsert ) -> execute();
                            } else {
                                try {
            ...
            public function execute($sql) {
                switch ($this->getConnectionType()) {
                    case 'ADODB' :
                        $rs = $this->db->Execute ( $sql ); // Execute the query and get the empty recordset
                        if (! $rs) {
                            throw new \Exception ( "Adodb error " . $this->getDb()->ErrorNo () . ": " . $this->getDb()->ErrorMsg () );
                        }
                        break;
                    case 'yii\\db\\Connection' :
                        $this->db->createCommand ( $sql )->execute ();
